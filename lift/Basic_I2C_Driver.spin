...
' ************** Added 18-08-2011 VEDS - Floris Duine - AT24C512 EEPROM Routines

PUB writeEeprom(SCL,device_address, register_msb, register_lsb, value)
  Initialize(SCL)
  start(SCL)
  write(SCL,device_address)
  write(SCL,register_msb)
  write(SCL,register_lsb)
  write(SCL,value)  
  stop (SCL)
  waitcnt((clkfreq/2)+cnt)

PUB readEeprom(SCL,device_address, register_msb, register_lsb) : value
  Initialize(SCL)
  start(SCL)
  write(SCL,device_address | 0)
  write(SCL,register_msb)
...
' ************** Added 08-11-2011 VEDS - Dennis Wokke - ADC routines

PUB readADCValue(SCL,device_address) : value | aTmp[3] , SDA
'' Reads the current ADC value from the ADS1000. Two bytes are read.
  value := 0
  
  start(SCL)
  write(SCL,device_address | Recv)
  ...
  value := aTmp[0] <- 16 + aTmp[1] <- 8 + aTmp[2]
    
  RETURN value

PUB readADCReg(SCL,device_address) : value | aTmp
'' Reads the current ADC value from the ADS1000. Three bytes are read.
'' Last byte is the config register, so that one is passed back.
  value := 0
...
PUB writeADCConfig(SCL,device_address, conf)
  start(SCL)
  write(SCL,device_address | Xmit)
  write(SCL,conf)
  stop(SCL)
